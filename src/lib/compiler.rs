...
impl From<&Command> for Vec<u8> {
    ...
    fn from(command: &Command) -> Self {
        ...
        match command {
            Command::MoveRight => vec![0x49, 0xff, 0xc5], // incr %r13
            Command::MoveLeft => vec![0x49, 0xff, 0xcd],  // decr %r13
            Command::Increment => vec![0x41, 0x80, 0x45, 0x00, 0x01], // addb $0x1,0x0(%r13)
            Command::Decrement => vec![0x41, 0x80, 0x6d, 0x00, 0x01], // subb   $0x1,0x0(%r13)
            Command::Print => vec![
...
impl Compiler {
    ...
    fn compile(&mut self) {
        ...
        for command in self.program.iter() {
            ...
            match command {
                Command::JumpForward => left_brackets_stack.push(self.machine_code.len()),
